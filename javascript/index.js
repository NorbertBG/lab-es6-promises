...
async function addInstruccions(recipe) {
  for (let i = 0; i < recipe.length -1; i++) {
    const step = await obtainInstruction(recipe, i);
    document.querySelector(`#${recipe}`).innerHTML += `<li>${step}</li>`;
  }
  document.querySelector("#broccoliImg").removeAttribute("hidden")
}

addInstruccions('broccoli');
